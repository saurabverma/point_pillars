...
def _flat_nested_json_dict_to_py_dict(json_dict, sep=".") -> dict:
	"""flat a nested json-like dict. this function make shadow copy.
	"""
	# Convert a nesetd json dict to simple python dict
	flatted = {}
	for k, v in json_dict.items():
		if isinstance(v, dict):
		...
		for _ in range(total_loop):
			...
			for step in tqdm(range(steps)):
				...
				if global_step % display_step == 0:
					...
					flatted_metrics = _flat_nested_json_dict_to_py_dict(metrics)
					flatted_summarys = _flat_nested_json_dict_to_py_dict(metrics, "/")
...
def _predict_kitti_to_file(net,
						   ...
						   lidar_input=False):
	# Predict net output, reformat output and save to a kitti-friendly file
