...
def _calculate_num_points_in_gt(data_path, infos, relative_path, remove_outside=True, num_features=4):
	for info in infos:
		...
		points_v = np.fromfile(
			v_path, dtype=np.float32, count=-1).reshape([-1, num_features]) # NOTE: load each kitti dataset scan points
		rect = info['calib/R0_rect']
		Trv2c = info['calib/Tr_velo_to_cam']
		P2 = info['calib/P2']
		if remove_outside:
			points_v = box_np_ops.remove_outside_points(points_v, rect, Trv2c, P2,
														info["img_shape"]) # NOTE: Remove the points which are not part of known ground-truth
		...
		# NOTE: Python lists are always pass by reference, thus no need to worry about returning the values
...
def create_kitti_info_file(data_path,
	...
	kitti_infos_train = kitti.get_kitti_image_info(
		...
		relative_path=relative_path) # NOTE: Get all useful info about each image such as TF, calibration, path to file, image shape
	_calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path) # Add num of points in ground-truth using call by reference approach
...
def _create_reduced_point_cloud(data_path,
	...
	for info in prog_bar(kitti_infos):
		...
		points_v = np.fromfile(
			str(v_path), dtype=np.float32, count=-1).reshape([-1, 4]) # Read the scan points
		rect = info['calib/R0_rect']
		P2 = info['calib/P2']
		Trv2c = info['calib/Tr_velo_to_cam']
		...
		points_v = box_np_ops.remove_outside_points(points_v, rect, Trv2c, P2,
													info["img_shape"]) # Remove points not part of Ground-truth
		# FIXME: Not sure if the 'rect', 'P2', etc. are indicative of all the ground-truth objects (such as cars)
...
def create_reduced_point_cloud(data_path,
	...
	# Store new dataset created by removing all lidar points which are not in same view as camera
	_create_reduced_point_cloud(data_path, train_info_path, save_path)
	_create_reduced_point_cloud(data_path, val_info_path, save_path)
	_create_reduced_point_cloud(data_path, test_info_path, save_path)
	if with_back: # Invert the points and then process as usual
...
def create_groundtruth_database(data_path,
	...
	# Start filling information in 'all_db_infos'
	for info in prog_bar(kitti_infos):
		...
		if not lidar_only:
			points = box_np_ops.remove_outside_points(points, rect, Trv2c, P2,
														info["img_shape"]) # FIXME: Shouldn't this be already done by 'create_reduced_point_cloud' function?
